   ...
   const [authors, setAuthors] = useState([]);
    ...
    useEffect(() => {
      const fetchAuthor = async () => {
        try {
          const response = await fetch('http://localhost:8080/api/author/getAllAuthor');
          if (!response.ok) {
  ...
  const handleSave = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setAuthors(authors.map((author) => (author.authorId === updatedData.authorId ? updatedData : author)));
        setEditAuthor(null);
        setUpdatedAuthor({
        });
      } else {
    ...
    const handleDelete = async (authorId) => {
      ...
      if (confirmDelete) {
        try {
          const response = await fetch(`http://localhost:8080/api/author/delete/${authorId}`, {
            method: 'DELETE',
          });
          if (response.ok) {
        ...
        <div className="author-content">
          ...
          {showAddForm && (
            <div className="add-author-form">
              ...
              />
              <button onClick={handleAddAuthor} className="btn-save">Save</button>
            </div>
          )}
                  ...
                  {authors.map((author) => (
                    ...
                    >
                      ...
                      <td className="author-data">
                        ...
                        <button className="btn-delete" onClick={()=>handleDelete(author.authorId)}
                        >Delete</button>
                      </td>
                    </tr>
                  ))} 
